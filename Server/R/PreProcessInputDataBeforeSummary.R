...
PreProcessInputDataBeforeSummary <- function(inputData, seed = NULL)
{
  ...
  for (colName in charColNames) {
    inputData[get(colName) %chin% c("UNK", ""), (colName) := NA_character_]
  }
  ...
  inputData[, AIDS := factor(ifelse(
    !is.na(DateOfAIDSDiagnosisISODate),
    ifelse(DateOfAIDSDiagnosisISODate <= DateOfDiagnosisISODate, "AIDS-Yes", "AIDS-No"),
    "AIDS-No"))
  ]
  ...
  inputData[, ":="(
    NotificationTime = year(DateOfNotificationISODate) + 1/4 * quarter(DateOfNotificationISODate),
    DiagnosisTime = year(DateOfDiagnosisISODate) + 1/4 * quarter(DateOfDiagnosisISODate)
  )]
